...
@endif
  ...
  <form action="{{route('ordenador.update', $ordenador)}}" method="POST">
    @csrf
    @method('PUT')
    <!-- Nombre del ordenador -->
    <div>
      <label for="nombre">Nombre ordenador:</label>
      <input type="text" id="nombre" name="nombre" value="{{$ordenador->nombre}}">
    </div>

    <!-- Descripción del ordenador -->
    <div>
      <label for="descripcion">Descripcion:</label>
      <textarea id="descripcion" name="descripcion" rows="4">{{$ordenador->descripcion}}</textarea>
    </div>
